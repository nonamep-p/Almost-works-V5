    ...
    for guild in bot.guilds:
        try:
            ...
            if channel:
                ...
                try:
                    await msg.delete()
                    logger.info(f"Auto-deleted startup message in {guild.name}")
                except discord.NotFound:
                    pass  # Message already deleted
                except discord.Forbidden:
                    logger.warning(f"No permission to delete startup message in {guild.name}")
            else:
    ...
    for guild in bot.guilds:
        try:
            ...
            if channel:
                ...
                asyncio.create_task(delete_message_after_delay(msg, 10, guild.name))
            else:
